...
PulseAudioSoundManager::PulseAudioSoundManager() : SoundManagerBase()
{
}
...
bool PulseAudioSoundManager::init() {
	...
	std::cerr << "pa_context_connect:" << pa_strerror(ret) << "\n";
	if (ret != PA_OK)
...
void PulseAudioSoundManager::start(bool isEnabled)
{
	...
	if (m_isEnabled)
	{
		...
		{
			pa_threaded_mainloop_lock(m_main_loop);
			m_stream = pa_stream_new(m_context, "RGBLights", &m_ss, nullptr);
			...
			if (m_device > 0 && m_device < m_devices.size()) {
				...
				std::cerr << "Pulseaudio device:" << m_devices[m_device].first << "\n";
			} else {
				std::cerr << "Pulseaudio device: Default device\n";
			}

			ret = pa_stream_connect_record(m_stream, dev.length() ? dev.c_str() : nullptr, &buffer_attr, PA_STREAM_ADJUST_LATENCY);

			if (ret != PA_OK) {
				std::cerr << "Pulseaudio failed to connect to device" << (dev.length() ? dev : "Default device") << "\n";
				goto unlock_and_fail;
			}
			...
			for(;;) {
				...
				if (stream_state == PA_STREAM_FAILED) {
					std::cerr << "Pulseaudio stream failed to connect to device" << dev << "\n";
					goto unlock_and_fail;
				}
				pa_threaded_mainloop_wait(m_main_loop);
			}
	...
	if (m_pa_ready > 1) {
		if (m_context) {
			...
			if (ret != PA_OK) { // Most likely got PA_ERR_BADSTATE so restart the whole thing
				std::cerr << "Pulseaudio reconnect failed:" << ret << pa_strerror(ret) << "\n";
				Uninit();
			}
		}
...
void PulseAudioSoundManager::stream_read_cb (pa_stream *p, size_t nbytes, void *userdata)
{
	...
	ret = pa_stream_drop(p);
	if (ret != PA_OK)
		std::cerr << __func__ << ": pa_stream_drop:" << pa_strerror(ret) << "\n";
}
