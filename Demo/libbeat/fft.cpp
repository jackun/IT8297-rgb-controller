...
{
    this->m_size=size;
    m_magnitude = new float[size/2];
    m_maxMagnitude=0;
    /*We should use fftw_malloc instead of new since fftw_malloc aligns the memory for optimal speed*/
    m_outputSignal = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex) * size);
    m_inputSignal = new float[size];
    m_SoundBuffer = NULL;
...
FFT::~FFT()
{
    ...
    delete [] m_magnitude;
    fftwf_free(m_outputSignal);
}
void FFT::processData()
{
    if (!m_SoundBuffer)
    {
        std::cerr << "libbeat: No Soundbuffer has been set!\n";
        return;
    }
    //This has to happen prior to any initialisation of the input array
    fftwf_plan plan_forward;
    plan_forward = fftwf_plan_dft_r2c_1d(m_size, m_inputSignal, m_outputSignal , 0);
    ...
    case(NO_WINDOW):
        ...
        {
            m_inputSignal[i]=m_SoundBuffer[i];
        }
        break;
    case(HANNING):
        for(uint16_t i=0; i<m_size; i++)
        {
            m_inputSignal[i] = m_SoundBuffer[i] * 0.5*(1.00-cos((2*M_PI*i)/(m_size-1)));
        }
        break;
    case(BLACKMAN):
        for(uint16_t i=0; i<m_size; i++)
        {
            ...
            double a1 = 0.5;
            double a2 = 0.16/2;
            m_inputSignal[i] = m_SoundBuffer[i] * a0-a1*cos((2*M_PI*i)/(m_size-1))+a2*cos((4*M_PI*i)/(m_size-1));
        }
        break;
    }
